...
exports.authentication = async (req, res) => {
    ...
    if ((APP_ID && APP_ID.length <= 36) && APP_ENV) {
        if (reqdata['username'] && reqdata['password']) {
            ...
            let decryptPass;
            try {
                decryptPass = CryptoJSAesJson.decrypt(JSON.stringify(reqdata['password']), public_key);
            } catch (error) {
            ...
            if (role_user.consumer_key) {
                if (decryptPass) {
                    ...
                    const account_user = { email: reqdata['username'], password: decryptPass };
                    const app_device = { APP_ID, APP_ENV };
                    let results = await CheckAccount(account_user, role_user, app_device, get_consumer_key, public_key);
                    response = results;
...
const CheckAccount = async (account, role_user, app_device, consumer_key, public_key) => {
    let results;
    //Get Base URI
    ...
    const isExistingUser = await FindAccountSession(app_device.APP_ID, app_device.APP_ENV, account.email);
    if (isExistingUser) {
        const delete_user = await SessionUserAccount.destroy({
            where: {
                [Sequelize.Op.or]: [
                    { [Sequelize.Op.and]: [{ App_ID: app_device.APP_ID }] },
                    { [Sequelize.Op.and]: [{ UserID: account.email }, { AppEnvironment: app_device.APP_ENV }] },
                ],
            },
        })
    }
    ...
    if (GetToken.result !== "error") {
        ...
        const msgSuccess = { internal: "Success sign-in.", user: "Success sign-in. Welcome " + dataTokenDB.UserID };
        results = { identity: identity_user, secretkey: secret_key, message: msgSuccess, result: "success" };
    } else {
        const msgError = await FindAnErrorMessage("01003");
        results = { message: (GetToken.message) ? GetToken.message : msgError, result: "error" }
    }
...
const GenerateSecretKey = (mail, public_key) => {
    ...
    const secretKey = require('crypto').createHash('sha256').update(encodeScKey).digest('hex');
...
const ExecuteLoginAPI = async (data, uri, headers) => {
    ...
    let responses = await axios(config)
        ...
        .catch(function (e) {
            results = { "message": { internal: e.response.data.error_description, user: e.response.data.error_description }, result: "error" }
            return results;
        })
